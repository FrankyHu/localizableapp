...
-(void) parse:(NSString *)filePath
{
	...
	//Pass 2
	for(id x in array){
		s = [NSString stringWithContentsOfFile:x encoding:NSUTF8StringEncoding error:nil];
		int i, j;
		//Looking for LFLSTR
		...
		for (i = 0 ;i < [s length] ;i++ ) {
			if ([s characterAtIndex:i] == 'L'){
				if((i + 7) < [s length] && [[s substringWithRange:NSMakeRange(i, 10)] isEqualToString:@"LFLSTR2(@\""]) {
					NSMutableArray *LFLSTR2 = [NSMutableArray new];
					...
					i = i + j;
					[LFLSTR2 addObject:param2];
					[_LFLSTR2list addObject:LFLSTR2];
				}
			}
		}
	}
	//Merge two arrays to one
	for (id x in _LFLSTRlist) {
...
- (void)awakeFromNib
{
	...
	[self parse:[NSHomeDirectory() stringByAppendingPathComponent:  @"Documents/RamdomApp/"]];
}
